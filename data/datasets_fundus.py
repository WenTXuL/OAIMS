...
from .transforms import get_interactive_transforms
...
class FundusDataset(Dataset):
    ...
    def __init__(self, images_path, masks_path, transform=False, return_path=False):
        ...
        if transform == 'interactive':
            self.transform = get_interactive_transforms()
        else:
            self.transform = None
...
def get_data(img_dir, mask_dir, transform=False, return_path=False):

    x = sorted(glob(f"{img_dir}/*"))
    y = sorted(glob(f"{mask_dir}/*"))
    print(f"Loaded {len(x)} images")
    return FundusDataset(x, y, transform=transform, return_path=return_path)
