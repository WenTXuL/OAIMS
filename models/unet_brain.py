...
def generate_guidance_maps_binary(images, points, labels, sigma=5):

    B, _, H, W = images.shape

    foreground = torch.zeros(B, H, W, dtype=torch.float32, device=images.device)
    background = torch.zeros_like(foreground)

    if points is None:
        return torch.cat([images, foreground.unsqueeze(1), background.unsqueeze(1)], dim=1)
    ...
    smoothed = []
    for signal in [foreground, background]:
...
class UNet(nn.Module):
    """U-Net with binary click guidance maps.

    Input: 1-channel grayscale + 2 Gaussian guidance maps (foreground, background)
    Output: 2-class segmentation map
    """
    ...
    def forward(self, images, points, labels, train_attention=True):
        x = generate_guidance_maps_binary(images, points, labels, sigma=5)
